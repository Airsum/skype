...
function isPrime(num){
	//var flag = true; 不是素数的情况 flag=false;
	for (var i = 2; i < num; i++) {
...
//9、跨浏览器兼容ie8获取所有的子元素节点集合。(因为在ie8下node.children包含注释节点)
...
//12、跨浏览器兼容ie8实现事件监听
function addEvent(ele,event,callBack,flag){//ele:表示要注册的事件对象；event：事件本身；callBack回调函数
...
function replaceSensitChar(str,sensitChar,char){
	...
	return str.replace(reg,char);//return str.split(sensitChar).join(char);
}